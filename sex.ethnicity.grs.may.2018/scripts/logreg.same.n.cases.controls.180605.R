...
logreg <- function(pheno, snp_groups, eth_groups, sex_groups, case_columns) {
	...
	results_df <- data.frame("snp_group" = character(), "eth_group" = character(), 
	...
	for (snp_group in snp_groups) {
		...
		df <- merge(x = scores, y = pheno, by.x = "IID", by.y = "ID")
        	...
        	for (eth_group in eth_groups) {
					...
					other_sex_df <- subset(eth_df, eth_df$Submitted_Gender == "M")
                        	} else if (sex_group == "men") {
                                	sex_df <- subset(eth_df, eth_df$Submitted_Gender == "M")
					other_sex_df <- subset(eth_df, eth_df$Submitted_Gender == "F")
                        	}
				...
				for (case_column in case_columns) {
					for (extra_adjustment in c("-", "smoking")) {
						...
						n_excluded <- nrow(sex_df[sex_df[, case_column] == 2, ])
						case_df <- sex_df[sex_df[, case_column] != 2, ]
						case_df[, case_column] <- factor(case_df[, case_column])

						#Get the minimum number of cases and controls between the sexes
						other_case_df <- other_sex_df[other_sex_df[, case_column] != 2, ]
						min_n_cases <- min(nrow(case_df[case_df[, case_column] == 1, ]), 
								nrow(other_case_df[other_case_df[, case_column] == 1, ])) 

						min_n_controls <- min(nrow(case_df[case_df[, case_column] == 0, ]),
                                	                        nrow(other_case_df[other_case_df[, case_column] == 0, ]))
					
						#Extract a random sample from the case_df to only contain the minimum N
						#for cases and controls
						set.seed(42)
						correct_n_cases_df <- case_df[case_df$IID %in% sample(case_df[case_df[, case_column] == 1, "IID"], min_n_cases, replace = F), ]				
						set.seed(42)
						correct_n_controls_df <- case_df[case_df$IID %in% sample(case_df[case_df[, case_column] == 0, "IID"], min_n_controls, replace = F), ]
					
						case_df <- rbind(correct_n_cases_df, correct_n_controls_df)
						...
						current_row <- data.frame("snp_group" = snp_group, "eth_group" = eth_group, "sex_group" = sex_group, 
...
snp_groups <- c("bmi.eur.comb.pulit.sig", "whr.eur.comb.pulit.sig", "whradjbmi.eur.comb.pulit.sig", 
		"bmi.eur.men.pulit.sig", "whr.eur.men.pulit.sig", 
		"whradjbmi.eur.men.pulit.sig", "bmi.eur.women.pulit.sig", 
...
case_columns <- c("cad_cases", "stroke_cases", "copd_cases", 
		...
		"renal_failure_cases", "t2d_cases_prob", "t2d_cases_probposs", "t1d_cases_prob", 
		"t1d_cases_probposs", "any_infertility_cases", "nafld_cases", "cld_cases", "smoker_cases", 
...
write.table(results_df, "../results.logistic.regressions.180514/log.results.table.same.n.cases.controls.180627.EXTRA.txt", 
		row.names = F, sep = "\t", quote = F)

sink("../results.logistic.regressions.180514/log.results.table.same.n.cases.controls.180627.WARNINGS.txt", append = F)
print(warnings())
sink()
...
write.table(results_df, "../results.logistic.regressions.180514/log.results.table.same.n.cases.controls.180627.txt",
                row.names = F, sep = "\t", quote = F)
